<template>
  <div class="bg-gray-100 text-gray-800 font-sans px-6 py-8 max-w-5xl mx-auto rounded shadow">
    ...
    <!-- Hero sekcija -->
    <section
      class="grid md:grid-cols-2 gap-6 items-center mb-12"
      v-if="data.hero_image || isNonEmpty(data.hero_title) || isNonEmpty(data.hero_subtitle)"
    >
      <div>
        <h2 class="text-3xl font-bold mb-2" v-if="isNonEmpty(data.hero_title)">{{ data.hero_title }}</h2>
        <p class="text-gray-600 italic whitespace-pre-line break-words" v-if="isNonEmpty(data.hero_subtitle)">
          {{ data.hero_subtitle }}
        </p>
      </div>
      <img
    ...
    <hr class="my-10 border-t border-dashed border-gray-300" v-if="isNonEmpty(data.description)" />

    <!-- Opis delatnosti -->
    <section v-if="isNonEmpty(data.description)" class="mb-10">
      <h2 class="text-xl font-semibold mb-2 text-indigo-600">📋 {{ $t('modernPreview.activityTitle') }}</h2>
    ...
    <hr
      class="my-10 border-t border-dashed border-gray-300"
      v-if="isNonEmpty(data.about_title) || isNonEmpty(data.about_text) || data.about_image"
    />

    <!-- O nama -->
    <section class="mb-12" v-if="isNonEmpty(data.about_title) || isNonEmpty(data.about_text) || data.about_image">
      <h2 class="text-2xl font-semibold text-blue-700 mb-4" v-if="isNonEmpty(data.about_title)">{{ data.about_title }}</h2>
      <div class="flex flex-col md:flex-row gap-6 items-center">
        <img
          ...
          loading="lazy"
        />
        <p class="text-sm leading-relaxed whitespace-pre-line break-words w-full" v-if="isNonEmpty(data.about_text)">
          {{ data.about_text }}
        </p>
      </div>
    </section>

    <hr class="my-10 border-t border-dashed border-gray-300" v-if="normalizedOfferItems.length || isNonEmpty(data.offer_title)" />
    ...
    <footer class="text-center text-sm text-gray-500 pt-4 border-t border-gray-300">
      ...
      <a href="https://express-web.express" target="_blank" rel="noopener" class="text-blue-600 hover:underline">
