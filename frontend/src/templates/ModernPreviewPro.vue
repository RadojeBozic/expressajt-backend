<template>
  <div id="page-top" class="bg-gray-100 text-gray-800 font-sans px-6 py-8 max-w-5xl mx-auto rounded shadow scroll-smooth">
    ...
    <!-- Hero -->
    <section class="grid md:grid-cols-2 gap-6 items-center mb-12 min-h-[60vh]" id="hero"
             v-if="isNonEmpty(data.hero_title) || isNonEmpty(data.hero_subtitle) || data.hero_image">
      <div>
        <h2 v-if="isNonEmpty(data.hero_title)" class="text-3xl font-bold mb-2">{{ data.hero_title }}</h2>
        <p v-if="isNonEmpty(data.hero_subtitle)" class="text-gray-600 italic whitespace-pre-line break-words">
          {{ data.hero_subtitle }}
        </p>
      </div>
      <img
    ...
    <!-- O nama -->
    <section class="mb-12" id="about" v-if="isNonEmpty(data.about_title) || isNonEmpty(data.about_text) || data.about_image">
      <h2 v-if="isNonEmpty(data.about_title)" class="text-2xl font-semibold text-blue-700 mb-4">{{ data.about_title }}</h2>
      <div class="flex flex-col md:flex-row gap-6 items-center">
        <img
          ...
          loading="lazy" decoding="async"
        />
        <p v-if="isNonEmpty(data.about_text)" class="text-sm leading-relaxed whitespace-pre-line break-words w-full">
          {{ data.about_text }}
        </p>
      </div>
    </section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="(item, index) in normalizedOfferItems" :key="index" class="flex flex-col items-center text-center">
          <img
            ...
            loading="lazy" decoding="async"
          />
          <p v-if="isNonEmpty(item.title)" class="mt-2 text-sm font-medium break-words whitespace-pre-line">
            {{ item.title }}
          </p>
        </div>
      </div>
    ...
    <section v-if="data.pdf_file" class="my-10">
      ...
      <div class="mt-2 text-sm">
        <a :href="getFileUrl(data.pdf_file)" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline">
          {{ $t('modernPreviewPro.openPdfInNewTab') || 'Otvori PDF u novom tabu' }}
        </a>
        <span class="mx-1 text-slate-400">|</span>
        <a :href="getFileUrl(data.pdf_file)" download class="text-blue-600 hover:underline">
          {{ $t('modernPreviewPro.downloadPdf') || 'Preuzmi PDF' }}
        </a>
      </div>
    </section>
    ...
    <footer class="text-center text-sm text-gray-500 pt-4 border-t border-gray-300 mt-10">
      ...
      <a href="#page-top" class="text-blue-600 hover:underline">{{ $t('modernPreviewPro.backToTop') }}</a>
    </footer>
  </div>
</template>
