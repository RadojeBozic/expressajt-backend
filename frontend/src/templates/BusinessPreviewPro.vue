<template>
  <div id="page-top" class="bg-white text-gray-900 font-sans px-6 py-8 max-w-5xl mx-auto shadow rounded-lg scroll-smooth">
    <!-- Header -->
    ...
    <!-- Hero -->
    <section class="mb-10 min-h-[50vh]" id="hero" v-if="isNonEmpty(data.hero_title) || isNonEmpty(data.hero_subtitle) || data.hero_image">
    ...
    <footer class="text-center text-sm text-gray-500 pt-4 border-t border-gray-300 mt-10">
      ...
      <a href="#page-top" class="text-purple-600 hover:underline">{{ $t('businessPreviewPro.backToTop') }}</a>
    </footer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getFileUrl(path) {
      ...
      const relative = trimmed.startsWith('storage/') ? trimmed : `storage/${trimmed}`

      if (!this.filesBaseUrl) return `/${relative}`
      return `${this.filesBaseUrl.replace(/\/+$/, '')}/${relative}`
    }
  }
}
</script>
