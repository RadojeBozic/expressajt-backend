<template>
  <div class="bg-white text-gray-800 font-mono p-6 max-w-6xl mx-auto shadow rounded-lg">
    ...
    <section class="mb-10" v-if="data.hero_image || isNonEmpty(data.hero_title) || isNonEmpty(data.hero_subtitle)">
      ...
      <p class="text-center text-gray-600 mb-4 whitespace-pre-line break-words" v-if="isNonEmpty(data.hero_subtitle)">
        {{ data.hero_subtitle }}
      </p>
    </section>

    <hr class="my-10 border-dashed border-gray-300" v-if="isNonEmpty(data.description)" />

    <!-- Opis delatnosti -->
    <section v-if="isNonEmpty(data.description)" class="mb-10">
      <h2 class="text-xl font-semibold mb-2 text-indigo-600">📋 {{ $t('galleryPreview.activityTitle') }}</h2>
    ...
    <section class="mb-10" v-if="isNonEmpty(data.about_title) || isNonEmpty(data.about_text) || data.about_image">
      <h2 class="text-xl font-semibold text-indigo-600 mb-4" v-if="isNonEmpty(data.about_title)">{{ data.about_title }}</h2>
      <div class="flex flex-col md:flex-row gap-6 items-center">
        <img
          ...
          loading="lazy"
        />
        <p class="text-sm leading-relaxed break-words whitespace-pre-line w-full" v-if="isNonEmpty(data.about_text)">
          {{ data.about_text }}
        </p>
      </div>
    </section>

    <hr class="my-10 border-dashed border-gray-300" v-if="normalizedOfferItems.length || isNonEmpty(data.offer_title)" />
