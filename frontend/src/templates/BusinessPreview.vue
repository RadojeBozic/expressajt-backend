<template>
  <div class="bg-slate-50 text-gray-900 font-sans px-6 py-8 max-w-5xl mx-auto shadow rounded-lg">
    ...
    <header class="flex items-center justify-between mb-8 border-b border-gray-300 pb-4">
      ...
      <div class="text-sm text-gray-600 text-center" v-if="isNonEmpty(data.email)">
        {{ data.email }}
      </div>
      <div class="text-sm text-gray-600 text-right" v-if="isNonEmpty(data.phone)">
        {{ data.phone }}
      </div>
    </header>
    ...
    <section v-if="isNonEmpty(data.description)" class="mb-10">
      <h2 class="text-xl font-semibold mb-2 text-purple-700">📋 {{ $t('businessPreview.activityTitle') }}</h2>
    ...
    <hr class="my-10 border-t border-dashed border-gray-300" v-if="normalizedOfferItems.length" />

    <!-- Naša ponuda -->
    <section v-if="normalizedOfferItems.length">
...
export default {
  ...
  methods: {
    ...
    isValidUrl(url) {
      try {
        if (!url) return false
        // pokušaj da napraviš URL; baciće grešku ako nije validan
        new URL(url)
        return true
      } catch {
        return false
    ...
    getFileUrl(path) {
      if (!path) return ''
      // već je pun URL? vrati kako jeste
      if (/^https?:\/\//i.test(path)) return path

      // ukloni leading '/' da ne dupliramo
      const clean = String(path).replace(/^\/+/, '')
      // ako već počinje sa 'storage/', ne dodaj opet
      const rel = clean.startsWith('storage/') ? clean : `storage/${clean}`

      // spoji sa bazom bez duplih '/'
