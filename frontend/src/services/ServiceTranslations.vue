<template>
  <div class="flex flex-col min-h-screen overflow-hidden supports-[overflow:clip]:overflow-clip">
        ...
        <div class="max-w-5xl mx-auto pb-20">
          <div class="max-w-3xl mx-auto text-center pb-12">
            <h2 class="h2 bg-clip-text text-transparent bg-gradient-to-r from-slate-200/60 via-slate-200 to-slate-200/60">
              {{ $t('service.translations.sectionTitle') }}
            </h2>
          </div>
          <div class="text-slate-400 space-y-6 max-w-3xl mx-auto">
            <p>{{ $t('service.translations.points.1') }}</p>
            <p>{{ $t('service.translations.points.2') }}</p>
           <p>{{ $t('service.translations.points.3') }}</p>
            <p>{{ $t('service.translations.points.4') }}</p>
            <p>{{ $t('service.translations.points.5') }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- CTA -->
    <section class="text-center py-12 bg-slate-900 border-t border-slate-700">
      <div class="max-w-xl mx-auto">
        <h2 class="text-xl font-semibold text-white mb-4">{{ $t('service.translations.ctaTitle') }}</h2>
        <p class="text-slate-400 mb-6">{{ $t('service.translations.ctaText') }}</p>
        <router-link
          to="/contact"
          class="inline-block bg-purple-600 hover:bg-purple-700 text-white font-bold px-6 py-2 rounded"
        >
          {{ $t('service.translations.ctaButton') }}
        </router-link>
      </div>
    </section>
...
export default {
  ...
  mounted() {
    console.log('Otvorena usluga: prevodi')
  },
}
</script>